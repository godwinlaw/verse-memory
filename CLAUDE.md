...
- `src/voice.js` — the spoken transcript (see Reciting aloud): `appendSpoken()`, the three undo steps (`undoPhrase`, `undoWord`, `clearSpoken`), `liveChunks()` reconciling against hand edits, `spokenCommand()`, and `hearFinal()` which is the whole of what one settled utterance does.
...
The whole design turns on one decision: **speaking fills `state.typed` and nothing else.** There is no second grade, no second award, no second exercise — `grading.js` and `srs.js` never learn how the words arrived, which is why reciting commits a verse with no rule anywhere agreeing that it should. Three modules, split by what each one needs to run:

- **`src/voice.js`** — pure, and the only part with real logic. A microphone hands over a finished phrase and offers no backspace, so every settled utterance is recorded as a chunk (`{ text, start }`) and the stack is what undo walks back down, in **three sizes**: `undoPhrase()` (the last thing said, gone whole), `undoWord()` (one word, for a misheard "lord"), `clearSpoken()`. The box stays editable by hand throughout, so `liveChunks()` reconciles the stack against the transcript **by position** — a chunk counts only while its words are still where it left them, and `undoPhrase()` quietly degrades to `undoWord()` when the tail has been edited. `spokenCommand()` recognises the same three steps said out loud ("scratch that", "backspace", "start over"), matching only a **whole** utterance so scripture is never mistaken for an instruction. `hearFinal()` is the whole of what one utterance does, and the only thing `App.hearRecitation` calls.
- **`src/recognizer.js`** — the browser seam, written like `firebase.js`: an optional overlay, and `availableEngines()` coming back empty just means the activity is a box you type in. Two engines behind one lifecycle of four callbacks (`onStatus` / `onPartial` / `onFinal` / `onError`) — `WEB_SPEECH` (the browser's own; nothing to download and it streams, so words appear mid-phrase) and `WHISPER` (works anywhere, nothing leaves the device, but a download and a phrase at a time). Which engines exist is asked **once, in `componentDidMount`**, and lives in `state.voice.engines`, so no view-model ever asks the window a question.
- **`src/whisper.js`** — transformers.js from a CDN plus mic capture, imported only if that engine is actually chosen. Whisper cannot stream, so rather than cutting members off on a timer it flushes on the **pause at the end of a phrase** (energy VAD) — which lands close to where a member would have drawn the boundary anyway, and those phrases are exactly the chunks undo hands back.

Three rules hold the rest of it together. The microphone belongs to **the card, not the session**: `resetCard()`, `submitCard()`, `leaveReview()` and the first-letter toggle all tear the recognizer down, and `hearRecitation()` re-checks the mode, the scaffold and the submitted flag, because an engine can deliver one last phrase after the member has moved on. The **interim tail is never in `typed`** — it is rendered beside the box as a ghost, so the grader cannot see a word the engine has not settled on. And the recognizer is **never started by anything but the member**; switching engine stops rather than restarts, since a Whisper start is a download.

`state.voice` is `{ engines, engine, status, loadPct, interim, chunks, error, command }`. Only `engine` is persisted (`mv.voiceEngine`, device-local like the difficulty levels); `resolveEngine()` is what decides whether a stored choice is still honourable on this device.
